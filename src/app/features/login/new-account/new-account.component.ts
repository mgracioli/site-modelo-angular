...
export class NewAccountComponent implements OnInit {
	...
	/************************************************************** 
	* Método que recupera os dados preenchidos nos campos do formulário e envia para o servidor.
	* Esse método é chamado sempre que se clica no botão de submit, o evento que dispara esse método está na tag <form> (na propriedade (ngSubmit)="onSubmit())
	* @param none
	* @returns void
	**************************************************************/	
	onSubmit() {
		if (this.form.valid) {  //o formulario só estará válido quando passar, com sucesso, por todos os Validators aplicados
	...
	* @param formGroup Formgroup no qual será adicionada a propriedade touched, com isso, o método applyCssFieldError() vai identificar que esses campos foram "tocados" e, com isso, aplicar o css de erro
	* @returns void
	**************************************************************/	
	...
	/************************************************************** 
	* Método para aplicar css de erro nos campos do formulário (borda vermelha no campo). 
	* Verifica se o campo está válido (se passou com sucesso pelos Vlidators) e se foi dado foco nele (touched) durante o preenchimento das inputs
	* @param field Campo do formulário em que se deseja aplicar a classe de erro
	* @returns class name : string
	**************************************************************/
	applyCssFieldError(field: string) {
		return {
			'field-error': !this.form.get(field)!.valid && this.form.get(field)!.touched //this.formulario.get(field) retorna uma instancia do campo correspondente do formulário criado lá no ngOnInit() - has-error é a classe do Bootstrap que vai ser aplicada no elemento caso o campo não esteja válido (caso não tenha passado com sucesso pelos Validators) e tenha sido tocado (touched)
		}
	}
	...
	* Esse método é ativado quando ocorre o blur da input de cep do formulário
	* @param none  
	* @returns void
	**************************************************************/
	findCEP() {
		...
		const cep = this.form.get('address.cep')
	...
	/************************************************************** 
	* Método para popular os dados do formulário.
	* Esse método é utilizado internamente pelo método findCEP()
	* @param data Recebe como parâmetro os dados retornados pela API de busca do CEP que serão usados para popular os campos do formulário
	* @returns void
	**************************************************************/
